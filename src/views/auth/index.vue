...
<template>
  <div class="relative w-h-full flex-center">
    <DynamicBackground />
    <div v-motion-slide-left class="absolute left-[10px] top-[10px] flex-y-center gap-[5px]">
      <img width="50" src="/logo.svg" alt="">
      <h3 class="text-6">
        {{ VITE_APP_TITLE }}
      </h3>
    </div>
    <div v-motion-slide-right class="absolute right-[10px] top-[10px] flex gap-[5px] rounded-sm bg-layer p-[5px]">
      <theme-palette />
      <theme-switch />
    </div>
    <n-card
      ...
      <div class="w-h-full flex overflow-hidden">
        <div class="w-h-full flex-1 flex-shrink-0 p-[10px]">
          <router-view v-slot="{ Component, route }">
            <component :is="Component" :key="route.fullPath" v-motion-slide-right />
          </router-view>
        </div>
      </div>
    </n-card>
